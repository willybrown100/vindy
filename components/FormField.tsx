...
const FormField = ({keyBoardType,value,title,changeText,otherStyles,placeholder}:formFieldProps) => {
    const [showPassword,setShowPassword]=useState(false)
  return (
    <View className={`  my-3 ${otherStyles}`}>
        <Text className='text-gray-100 m-1 capitalize'>{title}</Text>
        <View className='bg-black-100 border-2 flex-row items-center justify-between focus:border-secondary-default border-black-100 w-full rounded-xl '>
